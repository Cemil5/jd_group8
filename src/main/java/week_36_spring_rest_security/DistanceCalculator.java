...
*/
    ...
    public static void main(String[] args) {
        File resourcesDirectory = new File("src/main/java/week_36_spring_rest_security");
        String path = resourcesDirectory.getAbsolutePath() + "/input.txt";
        System.out.println("withScanner(path) = " + withScanner(path));
        System.out.println("withFilesReadAllLines(path) = " + withFilesReadAllLines(path));
        System.out.println("withStream(path) = " + withStream(path));
    }
    ...
    public static List<Double> withStream(String path) {
        ...
        try (var lines = Files.lines(Paths.get(path))) {
            result = lines.map(line -> {
                        String[] coordinates = line.replaceAll("[(),]", "").split("\\s+");
                        int x1 = Integer.parseInt(coordinates[0]);
                        int y1 = Integer.parseInt(coordinates[1]);
                        int x2 = Integer.parseInt(coordinates[2]);
                        int y2 = Integer.parseInt(coordinates[3]);
