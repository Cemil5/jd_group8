package week_22_SpringCore_SpringBoot;
...
//        int[] ints = {3, 0, 1};  // 0, 1, 3
//        int[] ints = {0, 1};
//        System.out.println(withSet(ints));
...
//    Follow up: Could you implement a solution using only O(1) extra space complexity and O(n) runtime complexity?
//    TC: O (n) SC: O(1)
    static int withSumming(int[] nums) {
        int n = nums.length;
...
Similarly , if we see , A^A^A^A.. with (even times)=0 and A^A^A.. with (odd times)=A
